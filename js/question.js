...
function addActiveQuestion(elem) {
  elem.classList.toggle('question-active');
  elem.parentElement.parentElement.classList.toggle('question-item__active');
}

questionList.addEventListener('click', function(event) {
  if (event.target.closest('.question__button')) {
    addActiveQuestion(event.target);
  }
});